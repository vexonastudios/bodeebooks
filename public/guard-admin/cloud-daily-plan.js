...
export function setupDailyPlan({ getSnapshot, mutate, navigate, editSubject, chooseSchool, endpoint = '/guard/dashboard/bridge/' }) {
  ...
  function updateControls() {
    ...
    const newDefault = isFamily && cards.some(card => card.preset === 'handwriting' && !card.subjectId && !captured?.rules.dailyPlanTemplate?.activities?.some(entry => entry.key === 'preset:handwriting'));
    ...
    familyText.textContent = isFamily
      ? newDefault && hasDefault ? 'Letters handwriting is now available in your default plan. Save default, then apply it to the children who should receive it.' : 'Save a reusable plan, then apply it to all or selected children. Changes here leave their current plans in place until you apply them.'
  ...
  function cardView(card) {
    ...
    if (summary !== 'Uses your activity settings' && card.preset !== 'handwriting') el.append(make('p', 'daily-plan-card-summary', summary));
