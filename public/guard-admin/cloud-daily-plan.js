...
export function setupDailyPlan({ getSnapshot, mutate, navigate, endpoint = '/guard/dashboard/bridge/' }) {
  const stylesheet = make('link'); stylesheet.rel = 'stylesheet'; stylesheet.href = '/guard-admin/cloud-daily-plan.css?v=20260910-board2'; document.head.append(stylesheet);
  ...
  const help = make('p', 'cloud-note', 'Drag an activity to a column, or choose “Move to.” Each child has their own plan. Activity on/off settings still apply.');
  ...
  section.append(head, help, controls, layout); document.querySelector('.main-content').append(section);
