...
import { DEFAULT_HANDWRITING, isHandwritingUrl, handwritingOptions, handwritingLink } from './cloud-handwriting-model.js';

export function cloudSubjectEdit(captured, subjectId, form) {
  const subject = captured.rules.subjects.find(item => item.id === subjectId), handwriting = form.get('kind') === 'handwriting', kind = handwriting ? 'website' : form.get('kind');
  const assignments = captured.students.flatMap(student => {
    ...
    if (active && handwriting && (!Number.isInteger(Number(form.get(`goal-${student.id}`))) || Number(form.get(`goal-${student.id}`)) < 1)) throw Error('Set at least one daily practice minute for each assigned child.');
  ...
  const provider = kind === 'website' && !handwriting ? form.get('portalProvider') : 'none';
  const url = handwriting ? handwritingLink({ style:form.get('handwriting-style'), letterCase:form.get('handwriting-case'), letters:form.get('handwriting-letters'), pause:Number(form.get('handwriting-pause')) }) : kind === 'website' ? form.get('url') : kind === 'activity' ? form.get('activityUrl') : '';
  return editSubjects(captured, subjectId, form.get('title'), url, false, assignments, {
    ...
    gradeCaptureEnabled: kind === 'website' && !handwriting && subject?.gradeCaptureEnabled === true,
    ...
    allowedDomains: kind === 'website' && !handwriting ? String(form.get('allowedDomains') || '').split(/[,\n\r]+/).map(value => value.trim()).filter(Boolean) : []
  });
}
...
export function editCloudSubject({ snapshot, editor, field, selectField, node, button, mutate, close, showError }, subject = null, { preset } = {}) {
  ...
  const handwriting = preset === 'handwriting' || isHandwritingUrl(subject?.url);
  const kind = handwriting ? 'handwriting' : subject?.kind || (subject?.url?.startsWith('app://') ? 'activity' : subject && !subject.url ? 'offline' : 'website');
  ...
  const kindField = selectField('Subject type', 'kind', [{ value: 'website', label: 'School website' }, { value: 'handwriting', label: 'Handwriting practice — Letters' }, { value: 'activity', label: 'Learning activity' }, { value: 'offline', label: 'Offline schoolwork' }], kind);
  const website = field('School website', 'url', kind === 'website' ? subject?.url || '' : '', { type: 'url', maxLength: 2048 });
  const practice = handwritingOptions(subject?.url) || DEFAULT_HANDWRITING;
  const practiceStyle = selectField('Style', 'handwriting-style', [{value:'print',label:'Print'},{value:'cursive',label:'Cursive'}], practice.style);
  const practiceCase = selectField('Letters or numbers', 'handwriting-case', [{value:'lower',label:'Lowercase · a–z'},{value:'upper',label:'Uppercase · A–Z'},{value:'digit',label:'Numbers · 0–9'}], practice.letterCase);
  const practiceLetters = field('Letters to practice, in order', 'handwriting-letters', practice.letters, { maxLength:100 });
  practiceLetters.querySelector('input').placeholder = 'a, c, d';
  const practicePause = numberField('Paper practice pause (seconds)', 'handwriting-pause', practice.pause, 120); practicePause.querySelector('input').min = '15';
  const practiceFields = node('div', 'cloud-handwriting-fields'); practiceFields.append(pair(practiceStyle, practiceCase), practiceLetters, practicePause,
    node('p', 'cloud-note', 'Watch each letter, practice on paper, then write it from memory. These letters repeat each day until you change them. Use another Handwriting subject for children practicing a different set.'));
  ...
  let websiteExtras = null, assignmentNote = null, assignmentGrid = null;
  function changeKind() {
    const selected = kindField.querySelector('select').value;
    practiceFields.hidden = selected !== 'handwriting';
    for (const input of practiceFields.querySelectorAll('input,select')) { input.disabled = selected !== 'handwriting'; input.required = selected === 'handwriting'; }
    ...
    if (assignmentNote) assignmentNote.textContent = selected === 'handwriting' ? 'Choose who practices these letters and set at least one daily minute. The full practice time is required.' : 'Choose who uses this subject. 0 minutes means no time goal; it does not mark schoolwork complete.';
    if (assignmentGrid) for (const input of assignmentGrid.querySelectorAll('input[name^="goal-"]')) input.min = selected === 'handwriting' ? '1' : '0';
  }
  ...
  const basics = panel('Subject details', 'notebook-pen', field('Name', 'title', subject?.title || (handwriting ? 'Handwriting' : '')), kindField, website, practiceFields, activity, provider, description, checkbox('Show this subject', 'active', subject?.active !== false));
  ...
  const currentIcon = subject?.icon || (handwriting ? 'pencil' : 'book-open');
  ...
  const color = field('Color', 'color', subject?.color || (handwriting ? '#34d399' : '#38bdf8'), { type: 'color' });
  ...
  const assignmentFields = node('fieldset', 'cloud-assignment-fields cloud-subject-assignments'), legend = node('legend', '', 'Children & daily goals'); legend.prepend(glyph('users')); assignmentNote = node('p', 'cloud-note'); assignmentFields.append(legend, assignmentNote);
  assignmentGrid = node('div', 'cloud-subject-children'); assignmentFields.append(assignmentGrid);
  for (const student of captured.students) {
    const current = subject ? assignmentFor(subject, student.id) : handwriting ? null : { dailyGoalMinutes: 30 }, row = node('div', 'cloud-assignment-row');
    ...
    const goal = numberField('Goal (minutes)', `goal-${student.id}`, current?.dailyGoalMinutes ?? (handwriting ? 10 : 30), 480), order = numberField('Child order', `order-${student.id}`, current?.displayOrder ?? subject?.displayOrder ?? 0);
