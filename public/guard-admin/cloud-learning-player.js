...
(() => {
  ...
  let player = null, ready = false, ended = false, feedbackTimer = null;
  function report() {
    ...
    window.bodeeVideo?.report({ position: Math.min(duration,Math.max(0,Math.floor(player.getCurrentTime() || 0))), duration, ended });
  }
  function seek(delta) {
  ...
  document.addEventListener('visibilitychange',() => { if (document.hidden && ready) { player.pauseVideo(); report(); } });
