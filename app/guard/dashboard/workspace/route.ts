...
export async function GET(request: Request) {
  ...
  try {
    ...
    const html = Object.entries(mediaPanels).reduce((html,[id,panel]) => html.replace(new RegExp(`<section class="tab-content" id="tab-${id}"[\\s\\S]*?<\\/section>`),panel),workspace.html)
      ...
      .replace('/guard-admin/cloud-messages.css"', '/guard-admin/cloud-messages.css?v=20260926-chat1"')
      ...
      .replace('<script type="module" src="/guard-admin/cloud-workspace.js"></script>', '<script type="module" src="/guard-admin/cloud-workspace.js?v=20260926-session1"></script>')
