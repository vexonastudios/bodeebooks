...
export async function GET(request: Request) {
  ...
  try {
    ...
    const html = Object.entries(mediaPanels).reduce((html,[id,panel]) => html.replace(new RegExp(`<section class="tab-content" id="tab-${id}"[\\s\\S]*?<\\/section>`),panel),workspace.html)
      ...
      .replace('/guard-admin/cloud-workspace.css"', '/guard-admin/cloud-workspace.css?v=20260914-calendar1"')
      ...
      .replace('<script type="module" src="/guard-admin/cloud-workspace.js"></script>', '<script type="module" src="/guard-admin/cloud-workspace.js?v=20260914-calendar1"></script>')
      .replace('</head>', '<link rel="stylesheet" href="/guard-admin/cloud-quick-unlock.css?v=20260913-popup1"></head>')
