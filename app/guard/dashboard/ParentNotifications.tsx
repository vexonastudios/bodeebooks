...
export default function ParentNotifications() {
  ...
  const [open,setOpen]=useState(false),[dismissed,setDismissed]=useState(''),[label,setLabel]=useState('This phone');
  ...
  useEffect(()=>{
    ...
    let unread:Unread[]=[],pendingMessage:string|null=null,checkingUnread=false,checkAgain=false;
  ...
  return <>
    ...
    {open&&<dialog ref={dialog} className={styles.installBackdrop} aria-labelledby="phone-notifications-title" onCancel={event=>{if(state.busy)event.preventDefault();else setOpen(false);}}>
      <section className={[styles.installCard,styles.notificationCard].join(' ')}>
        ...
        <p>Get alerts when your children send messages. Message contents stay off your lock screen.</p>
        {state.attention&&<p className={styles.notificationAttention} role="status">{state.attention}</p>}
        {state.iosInstall?<p>Add BodeeGuard to your Home Screen and open that icon to enable phone alerts.</p>:!state.supported?<p>Use the Home Screen app on iPhone or Chrome on Android for phone notifications.</p>:<>
        ...
        <p className={styles.notificationHint}>Turn off an old or lost phone here. This list is for your parent account.</p>
        ...
        <p className={styles.notificationHint}>Service acceptance does not confirm the phone displayed an alert. Focus, silent mode and phone settings control presentation.</p>
