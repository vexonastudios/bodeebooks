...
test('Worksheet transfer route forwards only the reviewed family snapshot and strips submitted rows or image authority',async()=>{
  ...
  await route.POST(request({action:'applyWorksheetTransfer',planId:deviceId,digest:'b'.repeat(64),recordings:[],coins:999}));assert.deepEqual(calls[1],{path:'/legacy/applyWorksheetTransfer',body:{planId:deviceId,digest:'b'.repeat(64)}});await route.POST(request({action:'rollbackWorksheetTransfer',planId:deviceId,rows:[]}));assert.deepEqual(calls[2],{path:'/legacy/rollbackWorksheetTransfer',body:{planId:deviceId}});
});

test('Poem transfer route forwards only the reviewed family snapshot and strips uploaded records or audio authority',async()=>{
  const calls=[],route=load('legacy/route.ts',{api:async(path,init)=>{calls.push({path,body:JSON.parse(init.body)});return{saved:true};}}),input={action:'planPoemTransfer',id:'a'.repeat(64),requestId:deviceId,studentId:deviceId,householdId:'forged',rows:[{}],audio:'forged',level:6};
  assert.equal((await route.POST(request(input))).status,200);assert.deepEqual(calls[0],{path:'/legacy/planPoemTransfer',body:{id:input.id,requestId:deviceId}});assert.equal((await load('legacy/route.ts',{authenticated:false}).POST(request(input))).status,401);assert.equal((await route.POST(request(input,{requestOrigin:'https://foreign.example'}))).status,403);
  await route.POST(request({action:'applyPoemTransfer',planId:deviceId,digest:'b'.repeat(64),recordings:[],coins:999}));assert.deepEqual(calls[1],{path:'/legacy/applyPoemTransfer',body:{planId:deviceId,digest:'b'.repeat(64)}});await route.POST(request({action:'rollbackPoemTransfer',planId:deviceId,rows:[]}));assert.deepEqual(calls[2],{path:'/legacy/rollbackPoemTransfer',body:{planId:deviceId}});
});
...
test('Typing transfer bridge only forwards archive, source identity and exact review IDs',async()=>{
  const calls=[];const route=load('legacy/route.ts',{api:async(path,init)=>{calls.push({path,body:JSON.parse(init.body)});return{};}});
  const input={action:'planTypingTransfer',id:'a'.repeat(64),sourceId:'original-child',requestId:deviceId,studentId:deviceId,coins:9999,householdId:'forged',rows:[{}]};
  assert.equal((await route.POST(request(input))).status,200);assert.deepEqual(calls[0],{path:'/legacy/planTypingTransfer',body:{id:input.id,sourceId:input.sourceId,requestId:deviceId}});
  assert.equal((await load('legacy/route.ts',{authenticated:false}).POST(request(input))).status,401);assert.equal((await route.POST(request(input,{requestOrigin:'https://foreign.example'}))).status,403);
  await route.POST(request({action:'applyTypingTransfer',planId:deviceId,digest:'b'.repeat(64),coins:9999}));assert.deepEqual(calls[1],{path:'/legacy/applyTypingTransfer',body:{planId:deviceId,digest:'b'.repeat(64)}});
  await route.POST(request({action:'rollbackTypingTransfer',planId:deviceId,rows:[]}));assert.deepEqual(calls[2],{path:'/legacy/rollbackTypingTransfer',body:{planId:deviceId}});
});

test('Daily transfer bridge only forwards archive, source identity and exact review IDs',async()=>{
  const calls=[];const route=load('legacy/route.ts',{api:async(path,init)=>{calls.push({path,body:JSON.parse(init.body)});return{};}});
  const input={action:'planDailyTransfer',id:'a'.repeat(64),sourceId:'original-child',requestId:deviceId,studentId:deviceId,coins:9999,householdId:'forged',rows:[{}]};
  assert.equal((await route.POST(request(input))).status,200);assert.deepEqual(calls[0],{path:'/legacy/planDailyTransfer',body:{id:input.id,sourceId:input.sourceId,requestId:deviceId}});
  assert.equal((await load('legacy/route.ts',{authenticated:false}).POST(request(input))).status,401);assert.equal((await route.POST(request(input,{requestOrigin:'https://foreign.example'}))).status,403);
  await route.POST(request({action:'applyDailyTransfer',planId:deviceId,digest:'b'.repeat(64),coins:9999}));assert.deepEqual(calls[1],{path:'/legacy/applyDailyTransfer',body:{planId:deviceId,digest:'b'.repeat(64)}});
  await route.POST(request({action:'rollbackDailyTransfer',planId:deviceId,rows:[]}));assert.deepEqual(calls[2],{path:'/legacy/rollbackDailyTransfer',body:{planId:deviceId}});
});
test('Practice transfer bridge only forwards archive, source identity and exact review IDs',async()=>{
  const calls=[];const route=load('legacy/route.ts',{api:async(path,init)=>{calls.push({path,body:JSON.parse(init.body)});return{};}});
  const input={action:'planPracticeTransfer',id:'a'.repeat(64),sourceId:'original-child',requestId:deviceId,studentId:deviceId,coins:9999,householdId:'forged',rows:[{}]};
  assert.equal((await route.POST(request(input))).status,200);assert.deepEqual(calls[0],{path:'/legacy/planPracticeTransfer',body:{id:input.id,sourceId:input.sourceId,requestId:deviceId}});
  assert.equal((await load('legacy/route.ts',{authenticated:false}).POST(request(input))).status,401);assert.equal((await route.POST(request(input,{requestOrigin:'https://foreign.example'}))).status,403);
...
test('Spelling transfer bridge only forwards archive, source identity and exact review IDs',async()=>{
  const calls=[];const route=load('legacy/route.ts',{api:async(path,init)=>{calls.push({path,body:JSON.parse(init.body)});return{};}});
  const input={action:'planSpellingTransfer',id:'a'.repeat(64),sourceId:'original-child',requestId:deviceId,studentId:deviceId,coins:9999,householdId:'forged',rows:[{}]};
  assert.equal((await route.POST(request(input))).status,200);assert.deepEqual(calls[0],{path:'/legacy/planSpellingTransfer',body:{id:input.id,sourceId:input.sourceId,requestId:deviceId}});
  assert.equal((await load('legacy/route.ts',{authenticated:false}).POST(request(input))).status,401);assert.equal((await route.POST(request(input,{requestOrigin:'https://foreign.example'}))).status,403);
  await route.POST(request({action:'applySpellingTransfer',planId:deviceId,digest:'b'.repeat(64),coins:9999}));assert.deepEqual(calls[1],{path:'/legacy/applySpellingTransfer',body:{planId:deviceId,digest:'b'.repeat(64)}});
  await route.POST(request({action:'rollbackSpellingTransfer',planId:deviceId,rows:[]}));assert.deepEqual(calls[2],{path:'/legacy/rollbackSpellingTransfer',body:{planId:deviceId}});
});
